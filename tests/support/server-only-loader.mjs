...
export async function resolve(specifier, context, nextResolve) {
  if (specifier === "server-only") return { shortCircuit: true, url: serverOnlyShim };
  if (specifier === "@neondatabase/serverless") return { shortCircuit: true, url: neonPostgresShim };
  if (specifier.startsWith("@/")) {
    const target = new URL(specifier.slice(2), sourceRoot);
    const typedTarget = new URL(`${specifier.slice(2)}.ts`, sourceRoot);
    if (existsSync(fileURLToPath(typedTarget))) return { shortCircuit: true, url: typedTarget.href };
