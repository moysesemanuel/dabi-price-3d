...
export class BillingReconciliationService {
  ...
  private recoverPaidInvoiceEffect(
    invoice: BillingInvoice,
  ): Promise<BillingReconciliationRunResult> {
    // Same routine the webhook duplicate path reuses (`invoice-effect-recovery.ts`):
    // a paid invoice with an incomplete effect claim is recovered identically
    // whether the cron finds it or a provider retry does.
    return recoverPaidInvoiceEffect(this.dependencies, invoice);
  }
